...
public class VehicleController {
    ...
    /** Khoảng cách an toàn tính từ đuôi xe trước đến đầu xe sau. */
    private static final double MIN_BUMPER_GAP = 28;
    private static final double LARGE_VEHICLE_BUMPER_GAP = 36;
    private static final double REACTION_DISTANCE_FACTOR = 2.35;
    private static final double SAFE_DISTANCE = 74;
    private static final double PROXIMITY_SLOW_DISTANCE = 132;
    private static final double PROXIMITY_STOP_DISTANCE = 58;
    ...
    private void applyTrafficRules() {
        for (Vehicle vehicle : vehicles) {
            ...
            if (frontVehicle != null
                    && distanceInFront(vehicle, frontVehicle) < followingSlowDistance(vehicle, frontVehicle)) {
    ...
    private void keepCommittedVehicleMoving(Vehicle vehicle) {
        ...
        if (frontVehicle != null) {
            ...
            if (gap >= 0 && gap < followingSlowDistance(vehicle, frontVehicle)) {
                handleTooCloseFrontVehicle(vehicle, frontVehicle);
                if (blockedVehicles.contains(vehicle) || laneChangingVehicles.containsKey(vehicle)) {
                    return;
                }
    ...
    private Vehicle findNearestFrontVehicle(Vehicle vehicle) {
        return vehicles.stream()
                ...
                .filter(other -> lateralDistanceBetween(vehicle, other)
                        <= Math.max(LANE_KEEP_MARGIN * 2,
                                (vehicleWidth(vehicle) + vehicleWidth(other)) * 0.5 + 3.0))
    ...
    private void handleTooCloseFrontVehicle(Vehicle vehicle, Vehicle frontVehicle) {
        ...
        double hardGap = minimumFollowingCenterGap(vehicle, frontVehicle);
        double desiredGap = desiredFollowingCenterGap(vehicle, frontVehicle);
        double slowGap = followingSlowDistance(vehicle, frontVehicle);
        ...
        // Chỉ đổi làn khi xe trước chậm và khoảng cách đã giảm, nhưng vẫn phải
        // kiểm tra khoảng trống phía trước/phía sau của làn đích.
        boolean shouldChangeLane = gap < slowGap
                && gap > hardGap + 4.0
        ...
        if (gap <= hardGap + 1.0) {
            ...
            limitSpeedNearVehicle(vehicle, frontVehicle, gap);
            if (gap < desiredGap && vehicle.getSpeed() <= 0.08) {
                blockVehicle(vehicle);
            }
        }
    }
    ...
    private boolean isPositionSafeForVehicleAt(Vehicle vehicle, double x, double y) {
        for (Vehicle other : vehicles) {
            ...
            if (other.getDirection() == vehicle.getDirection()) {
                ...
                double minLongitudinal = minimumFollowingCenterGap(vehicle, other);
    ...
    private double minimumFollowingCenterGap(Vehicle follower, Vehicle leader) {
        double bumperGap = isLargeVehicleForGap(follower) || isLargeVehicleForGap(leader)
                ? LARGE_VEHICLE_BUMPER_GAP : MIN_BUMPER_GAP;
        return vehicleLength(follower) * 0.5 + vehicleLength(leader) * 0.5 + bumperGap;
    }

    private double desiredFollowingCenterGap(Vehicle follower, Vehicle leader) {
        return minimumFollowingCenterGap(follower, leader)
                + Math.max(0.0, follower.getSpeed()) * REACTION_DISTANCE_FACTOR;
    }

    private double followingSlowDistance(Vehicle follower, Vehicle leader) {
        return desiredFollowingCenterGap(follower, leader) + 58.0;
    }

    private boolean isLargeVehicleForGap(Vehicle vehicle) {
    ...
    private void limitSpeedNearVehicle(Vehicle vehicle, Vehicle other, double distance) {
        double hardGap = minimumFollowingCenterGap(vehicle, other);
        double desiredGap = desiredFollowingCenterGap(vehicle, other);
        double slowGap = followingSlowDistance(vehicle, other);

        if (distance <= hardGap + 1.0) {
            vehicle.stop();
            return;
        }

        // Trong vùng phản ứng, tốc độ xe sau được ghép dần về tốc độ xe trước.
        // Không dùng các ngưỡng cố định 50/70 nữa vì xe buýt và xe cứu hỏa dài hơn.
        double targetSpeed;
        if (distance < desiredGap) {
            double ratio = Math.max(0.0, Math.min(1.0,
                    (distance - hardGap) / Math.max(1.0, desiredGap - hardGap)));
            targetSpeed = Math.min(other.getSpeed(), vehicle.getMaxSpeed() * ratio * ratio);
        } else {
            double ratio = Math.max(0.0, Math.min(1.0,
                    (distance - desiredGap) / Math.max(1.0, slowGap - desiredGap)));
            targetSpeed = other.getSpeed() + (vehicle.getMaxSpeed() - other.getSpeed()) * ratio;
        }

        // Xe thường nhường xe buýt khi cùng nhập một làn.
        if (!(vehicle instanceof Bus) && other instanceof Bus && distance < slowGap) {
            targetSpeed = Math.min(targetSpeed, Math.max(0.55, other.getSpeed()));
        }
        vehicle.setSpeed(Math.min(vehicle.getSpeed(), Math.max(0.0, targetSpeed)));
    }
    ...
    private void wakeVehicleWhenPathIsClear(Vehicle vehicle) {
        ...
        if (front != null) {
            ...
            if (gap >= 0 && gap < desiredFollowingCenterGap(vehicle, front)) {
                return;
            }
        }
        ...
        for (Vehicle vehicle : vehicles) {
            if (vehicle.getSpeed() < 0.05) {
                ...
                boolean frontBlocked = front != null
                        ...
                        && distanceInFront(vehicle, front) < desiredFollowingCenterGap(vehicle, front);
    ...
    private void applyTurnIfNeeded(Vehicle vehicle) {
        ...
        if (turnChoice == TurnChoice.LEFT && !isEmergency(vehicle) && hasOpposingTrafficNearIntersection(vehicle)) {
            continueStraightWhenTurnIsBlocked(vehicle);
            return;
        }
    ...
    /**
     * Chỉ được gọi trong mô phỏng ngã tư. Nếu xe chưa thể bắt đầu cung rẽ
     * vì có xe đối diện hoặc làn thoát đang bị chiếm, lựa chọn của xe được
     * đổi sang đi thẳng. Xe không đứng chờ giữa vùng đánh lái và cũng không
     * bật xi-nhan rẽ ở các tick tiếp theo.
     */
    private void continueStraightWhenTurnIsBlocked(Vehicle vehicle) {
        turnChoices.put(vehicle, TurnChoice.STRAIGHT);
        turningVehicles.remove(vehicle);
        laneChangingVehicles.remove(vehicle);
        blockedVehicles.remove(vehicle);
        vehicle.clearIndicators();
    ...
    /**
     * Lớp bảo vệ cuối sau mỗi tick. Xe sau không được tiếp tục tăng tốc nếu
     * khoảng cách tâm đã nhỏ hơn khoảng cách an toàn động.
     */
    private void enforceSafeFollowingDistances() {
        for (Vehicle follower : vehicles) {
            Vehicle leader = findNearestFrontVehicle(follower);
            if (leader == null) continue;
            double gap = distanceInFront(follower, leader);
            if (gap < 0) continue;

            double hardGap = minimumFollowingCenterGap(follower, leader);
            double desiredGap = desiredFollowingCenterGap(follower, leader);
            if (gap <= hardGap + 1.0) {
                follower.stop();
                blockVehicle(follower);
            } else if (gap < desiredGap) {
                follower.setSpeed(Math.min(follower.getSpeed(), leader.getSpeed()));
            }
        }
    }
