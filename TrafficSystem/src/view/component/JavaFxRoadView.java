...
public class JavaFxRoadView {
    ...
    public void render(GraphicsContext graphics) {
        ...
        // Tách riêng từng kiểu nút giao để các thay đổi dành cho ngã tư/lưới
        // không thể vẽ đè hoặc thay đổi nhánh giữa của ngã ba.
        if (mapType == TrafficMapType.T_JUNCTION) {
            coverNorthArmForTJunction(graphics);
            correctSouthArmForTJunction(graphics);
            return;
        }
    ...
    private void coverNorthArmForTJunction(GraphicsContext graphics) {
        // Che hoàn toàn nhánh phía Bắc của ngã tư để tạo đúng ngã ba Đông - Tây - Nam.
        double coverX = CENTER_X - MAIN_ROAD_W / 2 - 20;
        double coverW = MAIN_ROAD_W + 40;
        double coverH = CENTER_Y - INTERSECTION / 2 + 2;
        graphics.setFill(Color.web("#d7d9dc"));
        graphics.fillRect(coverX, 0, coverW, coverH);
        renderCornerBlock(graphics, CENTER_X - 112, 30, 224, 138, "CÔNG VIÊN");
    }


    /**
     * Sửa nhánh giữa/phía Nam của ngã ba.
     *
     * Bản vẽ ngã tư dùng trục Bắc - Nam rộng 260 px với bốn làn mỗi chiều.
     * Khi che nhánh Bắc để tạo ngã ba, phần đường phía Nam vì thế bị rộng hơn
     * hai nhánh Đông - Tây.  Ngã ba sử dụng lại bề rộng 205 px để cả ba nhánh
     * đồng đều và chỉ còn ba làn cho mỗi chiều.
     */
    private void correctSouthArmForTJunction(GraphicsContext g) {
        double intersectionBottom = CENTER_Y + INTERSECTION / 2;
        double crosswalkY = intersectionBottom + 32;
        double crosswalkH = 28;
        double roadW = SIDE_ROAD_W;
        double roadX = CENTER_X - roadW / 2;
        double oldRoadX = CENTER_X - MAIN_ROAD_W / 2;

        // Xóa phần nhánh Bắc - Nam cũ quá rộng ở phía Nam.
        g.setFill(Color.web("#cfd2d5"));
        g.fillRect(oldRoadX - 18, intersectionBottom,
                MAIN_ROAD_W + 36, height - intersectionBottom);

        // Vỉa hè và phần chuyển tiếp bo nhẹ từ vùng nút giao sang đường 3 làn.
        g.setFill(Color.web("#c8cace"));
        g.fillRect(roadX - 18, intersectionBottom,
                roadW + 36, height - intersectionBottom);

        g.setFill(Color.web("#555b58"));
        g.fillPolygon(
                new double[]{CENTER_X - INTERSECTION / 2, CENTER_X + INTERSECTION / 2,
                        roadX + roadW, roadX + roadW, roadX, roadX},
                new double[]{intersectionBottom, intersectionBottom,
                        crosswalkY, height, height, crosswalkY},
                6);

        // Vạch qua đường được vẽ lại đúng theo bề rộng mới.
        drawCrosswalkHorizontal(g, roadX + 10, crosswalkY, roadW - 20, crosswalkH);

        // Dải phân cách giữa bắt đầu sau vạch qua đường, không đè lên zebra.
        double laneStartY = crosswalkY + crosswalkH + 12;
        g.setFill(Color.web("#333735"));
        g.fillRect(CENTER_X - 8, laneStartY, 16, height - laneStartY);
        g.setFill(Color.web("#f5df4d"));
        g.fillRect(CENTER_X - 3, laneStartY, 6, height - laneStartY);

        // Mỗi chiều có đúng 3 làn: chỉ cần 2 vạch phân làn mỗi bên.
        g.setStroke(Color.web("#e9eeee"));
        g.setLineWidth(2);
        g.setLineDashes(18, 16);
        double[] laneLines = {
                CENTER_X - 68, CENTER_X - 42,
                CENTER_X + 42, CENTER_X + 68
        };
        for (double x : laneLines) {
            g.strokeLine(x, laneStartY, x, height);
        }
    ...
    private void renderDiagonalVietnamRoad(GraphicsContext g) {
        // Nhánh thứ năm đi từ Tây Nam vào vùng giao lộ, đủ rộng cho hai chiều xe.
        g.setStroke(Color.web("#c8cace"));
        g.setLineWidth(116);
        g.strokeLine(-35, 735, 355, 445);
        ...
        g.strokeLine(-25, 716, 345, 440);
        g.strokeLine(-45, 750, 335, 468);
        g.setLineDashes(null);
    }
